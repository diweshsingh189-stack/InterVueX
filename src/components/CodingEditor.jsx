import React, { useState, useEffect } from 'react';
import { 
  Code2, 
  Play, 
  CheckCircle2, 
  XCircle, 
  RotateCcw, 
  Terminal, 
  Sparkles,
  Layers
} from 'lucide-react';

const DEFAULT_LANGUAGES = [
  { id: 'javascript', name: 'JavaScript (Node.js)' },
  { id: 'python', name: 'Python 3' },
  { id: 'java', name: 'Java' },
  { id: 'cpp', name: 'C++ (g++)' }
];

export default function CodingEditor({ 
  question, 
  initialCode = '', 
  onChangeCode, 
  onRunComplete 
}) {
  const [language, setLanguage] = useState('javascript');
  const [code, setCode] = useState(() => {
    return initialCode || question?.starterCode?.javascript || `// Write your solution here\nfunction solution() {\n  return true;\n}`;
  });
  const [isRunning, setIsRunning] = useState(false);
  const [testResults, setTestResults] = useState(null);
  const [activeTab, setActiveTab] = useState('testcases'); // 'testcases' | 'output'

  // Update starter code when question or language switches
  useEffect(() => {
    if (question?.starterCode && question.starterCode[language]) {
      setCode(question.starterCode[language]);
      onChangeCode?.(question.starterCode[language]);
    }
  }, [language, question]);

  const handleCodeChange = (newCode) => {
    setCode(newCode);
    onChangeCode?.(newCode);
  };

  const handleKeyDown = (e) => {
    // Handle tab key in code editor
    if (e.key === 'Tab') {
      e.preventDefault();
      const start = e.target.selectionStart;
      const end = e.target.selectionEnd;
      const updated = code.substring(0, start) + '  ' + code.substring(end);
      setCode(updated);
      onChangeCode?.(updated);
      setTimeout(() => {
        e.target.selectionStart = e.target.selectionEnd = start + 2;
      }, 0);
    }
  };

  const handleReset = () => {
    const starter = question?.starterCode?.[language] || '';
    setCode(starter);
    onChangeCode?.(starter);
    setTestResults(null);
  };

  const handleRunCode = () => {
    setIsRunning(true);
    setActiveTab('output');

    setTimeout(() => {
      // Mock execution sandbox testing standard test cases
      const testCases = question?.testCases || [
        { input: 'arr = [1, 2, 3]', expected: 'true' },
        { input: 'arr = []', expected: 'false' }
      ];

      // Simulate passing evaluation for well-formed logic
      const isSyntaxEmpty = !code.trim() || code.trim().length < 20;
      
      const results = testCases.map((tc, idx) => {
        const passed = !isSyntaxEmpty;
        return {
          caseNum: idx + 1,
          input: tc.input,
          expected: tc.expected,
          actual: passed ? tc.expected : 'null/undefined',
          passed: passed
        };
      });

      setTestResults(results);
      setIsRunning(false);
      onRunComplete?.(results);
    }, 600);
  };

  const allPassed = testResults && testResults.every(r => r.passed);

  return (
    <div className="card" style={{ padding: '0', overflow: 'hidden', border: '1px solid var(--border-subtle)', marginBottom: '1.5rem' }}>
      {/* Editor Header Bar */}
      <div style={{
        display: 'flex',
        flexWrap: 'wrap',
        justifyContent: 'space-between',
        alignItems: 'center',
        padding: '0.75rem 1.25rem',
        backgroundColor: 'var(--bg-secondary)',
        borderBottom: '1px solid var(--border-subtle)',
        gap: '0.75rem'
      }}>
        {/* Language selector */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <Code2 size={18} style={{ color: 'var(--accent-cyan)' }} />
          <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)' }}>Language:</span>
          <select
            value={language}
            onChange={(e) => setLanguage(e.target.value)}
            className="form-select"
            style={{
              padding: '0.35rem 0.75rem',
              fontSize: '0.825rem',
              width: 'auto',
              backgroundColor: 'var(--surface-card)',
              color: 'var(--text-primary)',
              borderRadius: 'var(--radius-md)'
            }}
          >
            {DEFAULT_LANGUAGES.map(lang => (
              <option key={lang.id} value={lang.id}>{lang.name}</option>
            ))}
          </select>
        </div>

        {/* Action buttons */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <button
            type="button"
            onClick={handleReset}
            className="btn btn-sm btn-ghost"
            title="Reset to starter template"
            style={{ fontSize: '0.8rem', padding: '0.35rem 0.65rem' }}
          >
            <RotateCcw size={14} /> Reset
          </button>
          <button
            type="button"
            onClick={handleRunCode}
            disabled={isRunning}
            className="btn btn-sm btn-secondary"
            style={{
              fontSize: '0.8rem',
              padding: '0.35rem 0.85rem',
              borderColor: 'var(--accent-cyan)',
              color: 'var(--accent-cyan)'
            }}
          >
            <Play size={14} /> {isRunning ? 'Executing...' : 'Run Code'}
          </button>
        </div>
      </div>

      {/* Code Textarea / Sandbox */}
      <div style={{ position: 'relative', backgroundColor: 'var(--bg-primary)' }}>
        <textarea
          value={code}
          onChange={(e) => handleCodeChange(e.target.value)}
          onKeyDown={handleKeyDown}
          spellCheck={false}
          style={{
            width: '100%',
            minHeight: '260px',
            backgroundColor: 'transparent',
            color: 'var(--text-primary)',
            fontFamily: 'var(--font-mono)',
            fontSize: '0.9rem',
            lineHeight: '1.6',
            padding: '1rem 1.25rem',
            border: 'none',
            outline: 'none',
            resize: 'vertical'
          }}
          placeholder="// Type your algorithm here..."
        />
      </div>

      {/* Test Cases & Console Output Panel */}
      <div style={{
        borderTop: '1px solid var(--border-subtle)',
        backgroundColor: 'var(--bg-secondary)',
        padding: '1rem 1.25rem'
      }}>
        <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.75rem' }}>
          <button
            type="button"
            onClick={() => setActiveTab('testcases')}
            className={`btn btn-sm ${activeTab === 'testcases' ? 'btn-secondary' : 'btn-ghost'}`}
            style={{ fontSize: '0.75rem', padding: '0.25rem 0.65rem' }}
          >
            <Layers size={13} /> Test Cases
          </button>
          <button
            type="button"
            onClick={() => setActiveTab('output')}
            className={`btn btn-sm ${activeTab === 'output' ? 'btn-secondary' : 'btn-ghost'}`}
            style={{ fontSize: '0.75rem', padding: '0.25rem 0.65rem' }}
          >
            <Terminal size={13} /> Output Console {testResults && `(${allPassed ? 'Passed' : 'Failed'})`}
          </button>
        </div>

        {activeTab === 'testcases' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {(question?.testCases || [
              { input: 'nums = [2,7,11,15], target = 9', expected: '[0, 1]' },
              { input: 'nums = [3,2,4], target = 6', expected: '[1, 2]' }
            ]).map((tc, idx) => (
              <div 
                key={idx}
                style={{
                  backgroundColor: 'var(--surface-card)',
                  border: '1px solid var(--border-subtle)',
                  borderRadius: 'var(--radius-sm)',
                  padding: '0.5rem 0.85rem',
                  fontSize: '0.8rem',
                  fontFamily: 'var(--font-mono)',
                  display: 'flex',
                  flexWrap: 'wrap',
                  justifyContent: 'space-between',
                  alignItems: 'center',
                  gap: '0.4rem'
                }}
              >
                <div style={{ wordBreak: 'break-word', minWidth: '180px', flex: '1 1 auto' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Case {idx + 1}: </span>
                  <span style={{ color: 'var(--text-primary)' }}>{tc.input}</span>
                </div>
                <div style={{ color: 'var(--accent-cyan)', whiteSpace: 'nowrap' }}>
                  Expected: {tc.expected}
                </div>
              </div>
            ))}
          </div>
        )}

        {activeTab === 'output' && (
          <div>
            {isRunning ? (
              <div style={{ fontSize: '0.825rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span className="live-dot" /> Compiling and executing test cases...
              </div>
            ) : testResults ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {testResults.map((r) => (
                  <div
                    key={r.caseNum}
                    style={{
                      backgroundColor: 'var(--surface-card)',
                      border: r.passed ? '1px solid rgba(6, 182, 212, 0.4)' : '1px solid rgba(239, 68, 68, 0.4)',
                      borderRadius: 'var(--radius-sm)',
                      padding: '0.6rem 0.85rem',
                      fontSize: '0.8rem',
                      fontFamily: 'var(--font-mono)'
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                      <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>Test Case {r.caseNum}</span>
                      <span style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '0.3rem',
                        color: r.passed ? 'var(--accent-cyan)' : '#EF4444',
                        fontWeight: 700
                      }}>
                        {r.passed ? <><CheckCircle2 size={14} /> Passed</> : <><XCircle size={14} /> Failed</>}
                      </span>
                    </div>
                    <div style={{ color: 'var(--text-muted)' }}>Input: {r.input}</div>
                    <div style={{ color: 'var(--text-muted)' }}>Output: {r.actual}</div>
                  </div>
                ))}
              </div>
            ) : (
              <div style={{ fontSize: '0.825rem', color: 'var(--text-muted)' }}>
                Click "Run Code" to compile and execute test cases against the sandbox.
              </div>
            )}
          </div>
        )}
      </div>
    </div>
  );
}
