...
Time: O(M * N), Space: O(M * N) recursion stack.`,
  ...
  // ==========================================
  // 9. AI / ML FUNDAMENTALS (10 Questions)
  // ==========================================
  {
    id: 'topic-aiml-1',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'AI/ML - Paradigms',
    question: 'Explain the fundamental differences between Supervised, Unsupervised, and Reinforcement Learning with practical industry use cases.',
    keyConcepts: ['Labeled data vs Unlabeled clusters', 'Reward function & environment agent', 'Classification vs Clustering vs Policy optimization', 'Feedback loops'],
    tips: 'Give distinct real-world examples: Fraud detection (supervised), Customer segmentation (unsupervised), Autonomous driving/Game playing (reinforcement).',
    idealAnswer: 'Supervised Learning trains on labeled inputs (X -> Y) for regression/classification (e.g., spam detection). Unsupervised Learning discovers latent patterns in unlabeled data without targets (e.g., K-Means customer segmentation, PCA). Reinforcement Learning trains an agent interacting with an environment to maximize cumulative reward through exploration and exploitation (e.g., AlphaGo, robotics).'
  },
  {
    id: 'topic-aiml-2',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'AI/ML - Model Evaluation & Generalization',
    question: 'What is the Bias-Variance Tradeoff, and how do L1 (Lasso) and L2 (Ridge) Regularization prevent overfitting?',
    keyConcepts: ['Underfitting (High Bias)', 'Overfitting (High Variance)', 'L1 penalty (|w| produces sparsity)', 'L2 penalty (w^2 shrinks weights)', 'Generalization error'],
    tips: 'Explain that total error = Bias^2 + Variance + Irreducible Error. Highlight how L1 works as feature selection.',
    idealAnswer: 'High bias causes underfitting due to overly simple assumptions; high variance causes overfitting by capturing noise. Regularization adds a penalty to the loss function: L1 (Lasso: λ∑|w|) penalizes absolute weight magnitude driving irrelevant weights strictly to zero (feature selection); L2 (Ridge: λ∑w^2) shrinks weights smoothly towards zero, preventing any single feature from dominating.'
  },
  {
    id: 'topic-aiml-3',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'AI/ML - Metrics & Validation',
    question: 'Why is Accuracy often a misleading metric for imbalanced datasets? Explain Precision, Recall, F1-Score, and ROC-AUC.',
    keyConcepts: ['Confusion Matrix (TP, FP, TN, FN)', 'Precision = TP / (TP + FP)', 'Recall (Sensitivity) = TP / (TP + FN)', 'F1 harmonic mean', 'ROC curve & AUC trade-off'],
    tips: 'Use the 99% negative cancer detection example to demonstrate why 99% accuracy is useless if recall is 0%.',
    idealAnswer: 'In imbalanced datasets (e.g., 99% non-fraud, 1% fraud), a trivial model predicting "no fraud" achieves 99% accuracy but catches zero fraud. Precision measures what fraction of positive predictions were correct (minimizing false alarms). Recall measures what fraction of actual positives were caught. F1-Score is the harmonic mean balancing both. ROC-AUC evaluates the classifier across all discrimination thresholds.'
  },
  {
    id: 'topic-aiml-4',
    subject: 'aiml',
    ...
    question: 'How does Gradient Descent optimize neural weights? Compare Batch GD, Stochastic GD (SGD), and Adam Optimizer.',
    keyConcepts: ['Loss gradient descent (-∇L)', 'Learning rate schedule', 'Batch GD vs Mini-batch vs SGD', 'Momentum & adaptive learning rates (RMSProp + Momentum in Adam)'],
    tips: 'Mention why pure Batch GD is computationally impossible on billion-row datasets and how Adam computes moving averages of gradients and squared gradients.',
    idealAnswer: 'Gradient Descent iteratively updates parameters θ = θ - α∇L(θ) in the direction of steepest loss descent. Batch GD calculates gradients over the entire dataset (slow, high memory). SGD updates on single random samples (noisy, escapes local minima). Adam (Adaptive Moment Estimation) combines momentum (first moment / mean) and RMSProp (second moment / uncentered variance) to dynamically adapt per-parameter learning rates.'
  },
  {
    id: 'topic-aiml-5',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'AI/ML - Ensemble Methods',
    question: 'Explain the difference between Bagging and Boosting. Why do Random Forests and Gradient Boosted Trees (XGBoost) excel in tabular data?',
    keyConcepts: ['Bootstrap Aggregating (Bagging: parallel trees)', 'Boosting (sequential residual learning)', 'Random Forest feature subsampling', 'XGBoost second-order Taylor expansion'],
    tips: 'Clarify that Bagging reduces variance while Boosting primarily reduces bias.',
    idealAnswer: 'Bagging (Bootstrap Aggregation) trains independent trees in parallel on random data subsets with replacement and averages predictions, reducing variance (e.g., Random Forest). Boosting trains trees sequentially, where each new tree corrects the residual errors of prior trees, reducing bias (e.g., XGBoost, LightGBM). Both excel on tabular data by handling non-linear interactions without feature scaling.'
  },
  {
    id: 'topic-aiml-6',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'AI/ML - Deep Learning & Backprop',
    question: 'Explain the Vanishing Gradient problem in Deep Neural Networks. How do ReLU activations and Residual Connections (ResNet) solve it?',
    keyConcepts: ['Chain rule of calculus', 'Sigmoid/Tanh derivative saturation (< 0.25)', 'ReLU non-saturating gradient (f\'(x) = 1 for x > 0)', 'Skip connections (y = F(x) + x)'],
    tips: 'Explain mathematical backpropagation: multiplying many gradients < 1 shrinks gradients exponentially towards 0 in early layers.',
    idealAnswer: 'During backpropagation, the chain rule multiplies weight derivatives. Sigmoid/Tanh derivatives peak at <= 0.25; across many layers, gradients vanish to near zero, stopping early layers from learning. ReLU solves this because its gradient is constant (1) for positive inputs. ResNet skip connections (y = F(x) + x) allow gradients to backpropagate directly through the identity addition without attenuation.'
  },
  {
    id: 'topic-aiml-7',
    subject: 'aiml',
    ...
    question: 'What is the difference between Standardization (Z-score) and Min-Max Normalization? When should each be applied?',
    keyConcepts: ['Z-score: (x - μ) / σ (mean 0, std 1)', 'Min-Max: (x - min) / (max - min) [0, 1]', 'Sensitivity to outliers', 'Distance-based algorithms (KNN, SVM, K-Means)'],
    tips: 'Mention that Tree-based models are scale-invariant, whereas gradient-based and distance-based models strictly require scaling.',
    idealAnswer: 'Min-Max Normalization scales features to a fixed [0, 1] range; it is sensitive to extreme outliers. Standardization (Z-score) transforms features to have mean=0 and variance=1 without bounding the range; it handles outliers much better. Use Min-Max for neural network image inputs [0, 1] and Standardization for algorithms assuming Gaussian distributions (PCA, Logistic Regression, SVM).'
  },
  {
    id: 'topic-aiml-8',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'AI/ML - Dimensionality Reduction',
    question: 'How does Principal Component Analysis (PCA) work under the hood? What are Eigenvectors and Covariance Matrices in PCA?',
    keyConcepts: ['Covariance matrix calculation', 'Eigenvalues & Eigenvectors decomposition', 'Orthogonal maximum variance projection', 'Curse of dimensionality'],
    tips: 'Clarify that PCA is an unsupervised linear transformation preserving global variance.',
    idealAnswer: 'PCA reduces dimensionality while retaining maximum variance: 1) Standardize data, 2) Compute the Covariance Matrix representing feature relationships, 3) Calculate Eigenvalues (variance magnitude) and Eigenvectors (directions of principal axes), 4) Sort eigenvectors by largest eigenvalues, 5) Project original data onto the top k orthogonal eigenvectors.'
  },
  {
    id: 'topic-aiml-9',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'AI/ML - Data Preprocessing',
    question: 'How do you handle severe class imbalance in real-world production machine learning pipelines?',
    keyConcepts: ['Resampling (SMOTE oversampling, Random undersampling)', 'Class-weighted loss functions', 'Focal Loss', 'Stratified K-Fold validation', 'Precision-Recall calibration'],
    tips: 'Highlight that oversampling must ONLY be performed on training folds, never before train-test split, to avoid data leakage.',
    idealAnswer: 'Strategies: 1) Data-level: Synthetic Minority Over-sampling (SMOTE) or Tomek links undersampling on training data only. 2) Algorithm-level: Cost-sensitive learning using class weights in loss functions (e.g., weighting rare fraud class 100x). 3) Focal Loss to down-weight easy examples. 4) Evaluation: Use Precision-Recall AUC and Stratified K-Fold splits.'
  },
  {
    id: 'topic-aiml-10',
    subject: 'aiml',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'AI/ML - MLOps & Data Drift',
    question: 'What is the difference between Data Drift, Concept Drift, and Covariate Shift in deployed production ML systems?',
    keyConcepts: ['Data Drift: P(X) changes', 'Concept Drift: P(Y|X) changes (relationship shifts)', 'Covariate Shift: P(X) distribution changes but P(Y|X) holds', 'Continuous monitoring (PSI, KS-Test)'],
    tips: 'Use an e-commerce Covid purchasing shift example to illustrate sudden concept drift.',
    idealAnswer: 'Data Drift occurs when input feature distributions P(X) change over time. Covariate Shift occurs when input distributions shift between training and serving, though the underlying mapping P(Y|X) remains constant. Concept Drift occurs when the statistical relationship between input features and target outputs P(Y|X) changes (e.g., user purchasing habits post-inflation), requiring model retraining and automated trigger alerts.'
  },

  // ==========================================
  // 10. GENERATIVE AI & LLMS (10 Questions)
  // ==========================================
  {
    id: 'topic-genai-1',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Generative AI - Transformer Architecture',
    question: 'How does the Self-Attention mechanism in the Transformer architecture calculate contextual relationships between tokens?',
    keyConcepts: ['Query (Q), Key (K), Value (V) matrices', 'Scaled Dot-Product: softmax((QK^T) / sqrt(d_k)) * V', 'Multi-Head Attention', 'Parallel token computation vs RNN recurrence'],
    tips: 'Explain why dividing by sqrt(d_k) is critical: it prevents dot-products from growing large and pushing softmax into near-zero gradient regions.',
    idealAnswer: 'Each input token embedding is projected into Query (Q), Key (K), and Value (V) vectors. Self-attention computes attention weights using Scaled Dot-Product Attention: Attention(Q,K,V) = softmax((Q * K^T) / √d_k) * V. The dot product measures semantic compatibility between all token pairs simultaneously; multiplying by V yields a contextualized vector representation. Multi-Head attention performs this across multiple representation subspaces in parallel.'
  },
  {
    id: 'topic-genai-2',
    subject: 'genai',
    ...
    question: 'Explain Retrieval-Augmented Generation (RAG). Contrast Naive RAG with Advanced RAG (HyDE, Re-ranking, Chunking strategies).',
    keyConcepts: ['Document chunking & embedding generation', 'Vector DB top-k similarity retrieval', 'Prompt augmentation with retrieved context', 'Advanced: Reranker (Cross-encoder), HyDE (Hypothetical Document Embeddings)'],
    tips: 'Contrast RAG with fine-tuning: RAG provides dynamic, real-time private data access with citations without expensive GPU retraining.',
    idealAnswer: 'RAG augments LLM prompts with relevant external data: 1) Documents are chunked and converted into vector embeddings, 2) User queries search a Vector DB for top-k similar chunks, 3) Retrieved chunks are injected into the LLM system prompt for grounded generation. Advanced RAG improves retrieval using Hierarchical Chunking, HyDE (generating hypothetical answers before search), and Cross-Encoder Re-ranking to score relevance.'
  },
  {
    id: 'topic-genai-3',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Generative AI - Vector DB & Embeddings',
    question: 'How do Vector Databases perform fast Approximate Nearest Neighbor (ANN) search over millions of high-dimensional embeddings?',
    keyConcepts: ['Cosine Similarity vs Euclidean Distance (L2)', 'Hierarchical Navigable Small World (HNSW) graphs', 'Inverted File Index (IVF)', 'Scalar / Product Quantization (PQ)'],
    tips: 'Explain why brute-force KNN search O(N * d) fails at scale and how HNSW graph skip-lists achieve logarithmic search complexity.',
    idealAnswer: 'Exact KNN requires computing distances across every vector O(N * d), which is too slow for production. Vector DBs (Pinecone, Milvus, Chroma) use ANN algorithms like HNSW (Hierarchical Navigable Small World graphs), creating multi-layer graphs where top layers allow fast routing leaps and bottom layers provide fine-grained nearest neighbor matching in O(log N) time, combined with Product Quantization to compress vector memory.'
  },
  {
    id: 'topic-genai-4',
    subject: 'genai',
    ...
    question: 'Compare Zero-Shot, Few-Shot, Chain-of-Thought (CoT), and ReAct prompting patterns with architectural trade-offs.',
    keyConcepts: ['Zero-shot (direct prompt)', 'Few-shot (in-context demonstrations)', 'Chain-of-Thought ("Step by Step" reasoning)', 'ReAct (Reasoning + Action + Observation loop)'],
    tips: 'Highlight that Chain-of-Thought unlocks multi-step arithmetic and logic by forcing the model to generate intermediate reasoning tokens.',
    idealAnswer: 'Zero-shot gives instructions without examples. Few-shot provides input-output demonstration pairs in-context, aligning format and tone. Chain-of-Thought (CoT) prompts the model to "think step by step," generating intermediate reasoning tokens before final answers. ReAct (Reason + Act) combines reasoning traces with external tool invocations (APIs, search) in an iterative observation loop.'
  },
  {
    id: 'topic-genai-5',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Generative AI - Fine-Tuning & PEFT',
    question: 'How do LoRA (Low-Rank Adaptation) and QLoRA enable efficient fine-tuning of 70B+ parameter models on consumer GPUs?',
    keyConcepts: ['Freezing base model weights', 'Decomposing weight delta ΔW into low-rank matrices A and B (W + B*A)', 'Rank r << d', '4-bit NormalFloat (NF4) quantization + double quantization in QLoRA'],
    tips: 'Explain that full fine-tuning requires 4x memory for optimizer states (Adam), whereas LoRA only trains <1% of parameters.',
    idealAnswer: 'Full fine-tuning requires updating all model weights and storing optimizer states, demanding massive VRAM. LoRA freezes the original weights W (d x k) and injects trainable low-rank decomposition matrices B (d x r) and A (r x k) where rank r << min(d,k), reducing trainable parameters by >99%. QLoRA quantizes the base model into 4-bit NormalFloat precision while computing gradients through 16-bit LoRA adapters, allowing 70B models to train on single GPUs.'
  },
  {
    id: 'topic-genai-6',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Generative AI - Inference Optimization',
    question: 'What causes latency in LLM text generation? Explain the Prefill Phase, Decode Phase, KV Caching, and Continuous Batching.',
    keyConcepts: ['Compute-bound Prefill phase', 'Memory bandwidth-bound Auto-regressive Decode phase (1 token/step)', 'KV Caching (storing Keys & Values of past tokens in VRAM)', 'PagedAttention / vLLM continuous batching'],
    tips: 'Explain why generating token-by-token is memory-bandwidth bound and how KV cache eliminates re-computing attention for historical tokens.',
    idealAnswer: 'LLM generation has two phases: 1) Prefill (processes entire prompt in parallel, compute-bound), 2) Decode (generates tokens autoregressively one by one, memory bandwidth-bound). KV Caching stores Key and Value tensors from past tokens in GPU VRAM so attention is only computed for the newest token. Continuous batching (vLLM / PagedAttention) dynamically inserts new requests as older requests finish without waiting for full batch completion.'
  },
  {
    id: 'topic-genai-7',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Generative AI - Safety & Evaluation',
    question: 'How do you detect, mitigate, and benchmark LLM Hallucinations in production enterprise systems?',
    keyConcepts: ['Groundedness & Faithfulness metrics (RAGAS framework)', 'Guardrails (NeMo, Llama Guard, JSON schema validation)', 'Chain-of-verification (CoVe)', 'Temperature & Top-p tuning'],
    tips: 'Emphasize temperature=0 for deterministic factual tasks and schema enforcement (like OpenAI Structured Outputs).',
    idealAnswer: 'Hallucination mitigation strategies: 1) RAG with strict prompt grounding ("Answer ONLY using context"), 2) Lower temperature (0.0 - 0.2) to reduce stochastic drift, 3) Automated evaluation using frameworks like RAGAS (measuring Faithfulness, Answer Relevance, and Context Recall), 4) Guardrails (input/output safety scanners), 5) Multi-agent critique loops where a second model verifies factual claims against citations.'
  },
  {
    id: 'topic-genai-8',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Generative AI - Alignment & RLHF',
    question: 'Explain Reinforcement Learning from Human Feedback (RLHF) vs Direct Preference Optimization (DPO) for model alignment.',
    keyConcepts: ['Supervised Fine-Tuning (SFT) baseline', 'Reward Model training on human preference pairs (win vs loss)', 'PPO (Proximal Policy Optimization) with KL-divergence penalty', 'DPO closed-form loss without separate reward model'],
    tips: 'Highlight that DPO optimizes alignment directly on preference data without needing a fragile RL training loop.',
    idealAnswer: 'RLHF aligns models with human intent in 3 steps: 1) Supervised Fine-Tuning (SFT), 2) Train a Reward Model on human preference pairs (chosen vs rejected), 3) Optimize the policy using PPO with a KL penalty to prevent reward hacking. DPO (Direct Preference Optimization) mathematically eliminates the complex PPO loop and separate reward model by directly optimizing a closed-form cross-entropy loss on preference pairs.'
  },
  {
    id: 'topic-genai-9',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Generative AI - Agents & Function Calling',
    question: 'How does LLM Function Calling / Tool Use work under the hood between the client application and the model?',
    keyConcepts: ['JSON Schema function declarations in system prompt', 'Model generates structured tool call arguments (stop generation)', 'Client executes tool (e.g. database query, API call)', 'Client feeds tool output back to model for final synthesis'],
    tips: 'Clarify that the LLM itself does NOT execute code or call APIs directly; it generates structured JSON parameters which the hosting server executes.',
    idealAnswer: '1) Developer provides JSON schemas describing available tools (parameters, types, descriptions). 2) The model analyzes user intent; if a tool is needed, it outputs a JSON payload with function name and arguments instead of normal text. 3) The client backend receives the payload, executes the actual code/API, and sends the raw result back as a "tool" role message. 4) The model synthesizes the tool result into a natural response.'
  },
  {
    id: 'topic-genai-10',
    subject: 'genai',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Generative AI - Tokenization & Context',
    question: 'How does Byte-Pair Encoding (BPE) tokenization work? What are Context Windows and RoPE (Rotary Position Embeddings)?',
    keyConcepts: ['Subword tokenization (BPE vocabulary building)', 'Token-to-word ratio (100 tokens ~ 75 words)', 'Rotary Position Embedding (RoPE: complex rotation for relative positions)', 'Context length scaling (YaRN, LongLoRA)'],
    tips: 'Explain why character tokenization produces too long sequences, while word tokenization creates massive out-of-vocabulary (OOV) tables.',
    idealAnswer: 'BPE starts with individual characters and iteratively merges the most frequently occurring byte/character pairs to build a subword vocabulary (e.g., Tiktoken), balancing sequence length and vocabulary size. RoPE (Rotary Position Embedding) encodes token positions by rotating query/key representations in complex vector space, preserving relative distances across long context windows (128k+ tokens) seamlessly.'
  },

  // ==========================================
  // 11. CLOUD COMPUTING (10 Questions)
  // ==========================================
  {
    id: 'topic-cloud-1',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'Cloud - Core Service Models',
    question: 'Explain IaaS, PaaS, SaaS, and Serverless (FaaS) with real-world examples and shared responsibility boundaries.',
    keyConcepts: ['IaaS (AWS EC2 - OS/app management)', 'PaaS (AWS Elastic Beanstalk / Heroku - runtime managed)', 'SaaS (Google Workspace / Salesforce - turnkey app)', 'FaaS / Serverless (AWS Lambda - event-driven code execution)'],
    tips: 'Illustrate with the "Pizza as a Service" analogy or cloud responsibility matrix (who manages hardware, OS, runtime, and data).',
    idealAnswer: 'IaaS (Infrastructure as a Service: EC2, Azure VMs) provides raw compute/storage; user manages OS, patches, and runtime. PaaS (Platform as a Service: App Engine, Heroku) manages OS, runtime, and scaling; user supplies application code. SaaS (Software as a Service: Slack, Microsoft 365) provides complete applications managed entirely by vendor. Serverless / FaaS (AWS Lambda) executes stateless event-triggered code with automatic scaling and zero idle cost.'
  },
  {
    id: 'topic-cloud-2',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Cloud - High Availability & Resiliency',
    question: 'How do you design a High-Availability (HA) and Disaster Recovery (DR) architecture? Explain RTO and RPO.',
    keyConcepts: ['RTO (Recovery Time Objective)', 'RPO (Recovery Point Objective)', 'Multi-AZ vs Multi-Region deployments', 'Active-Active vs Active-Passive (Pilot Light / Warm Standby)'],
    tips: 'Differentiate RPO (maximum acceptable data loss in time) from RTO (maximum acceptable downtime to restore service).',
    idealAnswer: 'RPO (Recovery Point Objective) is the maximum acceptable data loss measured in time; RTO (Recovery Time Objective) is the maximum tolerable downtime. HA architectures deploy across Multi-Availability Zones with load balancers and auto-scaling. DR strategies range from Backup & Restore (high RTO/RPO, low cost), Pilot Light (core DB synced), Warm Standby, to Multi-Region Active-Active (near zero RTO/RPO, high cost).'
  },
  {
    id: 'topic-cloud-3',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Cloud - Networking & VPC',
    question: 'Explain Virtual Private Cloud (VPC) architecture: Public Subnets, Private Subnets, NAT Gateways, and Security Groups vs NACLs.',
    keyConcepts: ['VPC CIDR block isolation', 'Public subnet (Internet Gateway route)', 'Private subnet (NAT Gateway outbound routing)', 'Security Groups (Stateful, instance level)', 'NACLs (Stateless, subnet level)'],
    tips: 'Remember: Security Groups are stateful (inbound allow automatically permits response), while NACLs are stateless.',
    idealAnswer: 'A VPC is an isolated virtual network in the cloud. Public subnets route directly to an Internet Gateway (used for Load Balancers/Bastion hosts). Private subnets house databases/microservices and reach the internet for outbound updates via a NAT Gateway. Security Groups are stateful firewalls at the instance/ENI level; Network Access Control Lists (NACLs) are stateless firewalls at the subnet boundary.'
  },
  {
    id: 'topic-cloud-4',
    subject: 'cloud',
    ...
    question: 'How does AWS Lambda work under the hood? What causes Cold Starts and how do you minimize cold start latency?',
    keyConcepts: ['MicroVM sandbox (Firecracker)', 'Cold start (container download + runtime init) vs Warm execution', 'Provisioned Concurrency', 'Package size optimization & lightweight runtimes (Node/Go/Rust)'],
    tips: 'Explain that cold starts occur when no idle execution environment exists to handle a newly arriving concurrent request.',
    idealAnswer: 'AWS Lambda provisions ephemeral MicroVM sandboxes (using Firecracker) on-demand. A Cold Start occurs when a new container must be downloaded, initialized with the runtime, and execute initialization code before processing the handler. To minimize cold starts: 1) Keep deployment packages minimal, 2) Use fast-booting runtimes (Node.js/Go/Rust), 3) Configure Provisioned Concurrency to keep pre-initialized warm instances ready.'
  },
  {
    id: 'topic-cloud-5',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Cloud - Storage & Consistency',
    question: 'Compare AWS S3 (Object Storage), EBS (Block Storage), and EFS (File Storage) across latency, sharing, and throughput use cases.',
    keyConcepts: ['S3 Object storage (HTTP REST, infinite scale, strong consistency)', 'EBS Block storage (low latency NVMe, single EC2 instance attachment)', 'EFS File storage (POSIX, NFSv4, shared multi-instance mount)'],
    tips: 'Mention that S3 is now strongly consistent for read-after-write operations across all regions.',
    idealAnswer: 'EBS (Elastic Block Store) provides low-latency block-level volumes formatted as local file systems for single EC2 instances (ideal for databases). EFS (Elastic File System) provides managed NFS shared storage mounted concurrently by hundreds of EC2/container instances. S3 (Simple Storage Service) is highly durable (11 9s) object storage accessed via HTTP APIs with lifecycle policies, suitable for static assets, backups, and data lakes.'
  },
  {
    id: 'topic-cloud-6',
    subject: 'cloud',
    ...
    question: 'What is the Principle of Least Privilege in Cloud IAM? Explain IAM Users, Roles, Policies, and STS Temporary Credentials.',
    keyConcepts: ['Least Privilege principle', 'IAM Policies (JSON statements: Effect, Action, Resource, Condition)', 'IAM Roles (assumeRole for EC2/Lambda without hardcoded keys)', 'AWS Security Token Service (STS)'],
    tips: 'Emphasize NEVER hardcoding access keys in code or Dockerfiles; always use IAM instance profiles / roles.',
    idealAnswer: 'The Principle of Least Privilege grants identities only the minimum necessary permissions required to perform their tasks. IAM Users represent long-term human identities; IAM Roles are assumed dynamically by services, applications, or federated users using AWS STS to generate short-lived, rotatable temporary credentials without hardcoding secrets.'
  },
  {
    id: 'topic-cloud-7',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Cloud - Auto Scaling & Load Balancing',
    question: 'How do Application Load Balancers (ALB) and Auto Scaling Groups (ASG) coordinate during sudden traffic spikes?',
    keyConcepts: ['Target groups & Health checks', 'Dynamic scaling policies (Target Tracking, Step scaling, Predictive)', 'ALB path-based routing & TLS termination', 'Cooldown periods & graceful connection draining'],
    tips: 'Explain target tracking scaling (e.g., maintaining 70% average CPU utilization) and connection draining during scale-in.',
    idealAnswer: 'The ALB distributes incoming HTTP/HTTPS traffic across healthy EC2/container instances in target groups and performs periodic health check pings. ASG monitors CloudWatch metrics (CPU, request count per target); when metrics breach thresholds, ASG launches new instances, attaches them to ALB target groups, and warms them up. During scale-in, ALB applies connection draining (deregistration delay) to finish active requests before instance termination.'
  },
  {
    id: 'topic-cloud-8',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Cloud - Databases & Replication',
    question: 'How does Amazon Aurora achieve 5x throughput over standard MySQL/PostgreSQL? Explain distributed storage and storage tier replication.',
    keyConcepts: ['Decoupled compute and storage architecture', '6-way storage replication across 3 AZs', 'Log-is-the-database paradigm (only write redo logs over network)', 'Sub-10ms read replica lag'],
    tips: 'Highlight that Aurora does not write dirty database pages to disk over the network; it only writes redo logs to the distributed storage fleet.',
    idealAnswer: 'Aurora decouples database compute from storage. Instead of writing full buffer pages across AZs, Aurora only sends redo log records across a purpose-built distributed storage fleet (replicated 6 ways across 3 AZs with 4/6 quorum writes). The storage nodes apply redo logs in parallel asynchronously, eliminating I/O bottlenecks and enabling instant crash recovery and sub-10ms read replica replication.'
  },
  {
    id: 'topic-cloud-9',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'Cloud - Cost Optimization (FinOps)',
    question: 'What strategies and pricing models do you use to reduce AWS/Cloud infrastructure costs without sacrificing performance or reliability?',
    keyConcepts: ['Savings Plans & Reserved Instances (1-3 yr commitment)', 'Spot Instances for fault-tolerant workloads', 'S3 Intelligent-Tiering & Lifecycle expiration', 'Rightsizing overprovisioned compute'],
    tips: 'Mention AWS Cost Explorer, AWS Compute Optimizer recommendations, and architectural changes like serverless/auto-shutdown for non-prod environments.',
    idealAnswer: '1) Pricing models: Use 1-3 year Reserved Instances or Compute Savings Plans for baseline 24/7 workloads (up to 72% discount); leverage Spot Instances for stateless batch/worker fleets (up to 90% discount). 2) Storage: S3 Intelligent-Tiering and lifecycle rules to transition old assets to Glacier. 3) Compute: Right-size instances using CloudWatch metrics and automate non-prod environment shutdowns off-hours.'
  },
  {
    id: 'topic-cloud-10',
    subject: 'cloud',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Cloud - Edge Computing & CDN',
    question: 'How do CloudFront / CDNs and Edge Functions (Lambda@Edge / Cloudflare Workers) accelerate global web applications?',
    keyConcepts: ['Edge locations & Point of Presence (PoP)', 'Cache-Control headers & TTL invalidation', 'Edge compute: request rewriting, auth at edge, geo-routing', 'Origin Shielding'],
    tips: 'Explain that executing logic at the edge terminates TLS and answers requests close to the user in single-digit milliseconds without hitting origin servers.',
    idealAnswer: 'A CDN (CloudFront, Cloudflare) caches static assets (JS, CSS, images, video) at hundreds of globally distributed Edge PoPs, terminating TLS close to end-users and serving cached content with single-digit millisecond latency. Edge Functions (Cloudflare Workers, Lambda@Edge) execute lightweight JavaScript/Wasm at edge nodes to perform header mutations, JWT authentication, A/B testing redirects, and dynamic personalization before hitting origin servers.'
  },

  // ==========================================
  // 12. DEVOPS & CI/CD (10 Questions)
  // ==========================================
  {
    id: 'topic-devops-1',
    subject: 'devops',
    ...
    question: 'Design an end-to-end production CI/CD pipeline using modern tools (GitHub Actions, Docker, Kubernetes). What stages are critical?',
    keyConcepts: ['Linting & Static Analysis (SonarQube)', 'Unit & Integration test execution', 'Docker container build & multi-stage caching', 'Security scans (Trivy/Snyk)', 'GitOps CD deployment (ArgoCD)'],
    tips: 'Distinguish Continuous Integration (automated build, test, scan) from Continuous Delivery (automated artifact staging) and Continuous Deployment (automated production release).',
    idealAnswer: 'Pipeline Stages: 1) Checkout & Lint/Typecheck, 2) Unit & Integration test execution with coverage thresholds, 3) Static Code Analysis & SAST security scanning, 4) Multi-stage Docker image build with layer caching, 5) Container vulnerability scanning (Trivy), 6) Push signed image to container registry, 7) GitOps CD tool (ArgoCD/Flux) detects image update and triggers rolling/canary deployment to Kubernetes cluster with automated health checks.'
  },
  {
    id: 'topic-devops-2',
    subject: 'devops',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'DevOps - Docker Internals',
    question: 'How do Docker Containers work under the hood? Contrast Linux Namespaces, Cgroups, and Union File Systems (OverlayFS).',
    keyConcepts: ['Linux Namespaces (isolation: pid, net, ipc, mnt, uts)', 'Control Groups (Cgroups: resource limits on CPU/RAM)', 'OverlayFS (Copy-on-Write layer stacking)', 'Containers vs VMs'],
    tips: 'Emphasize that containers share the host Linux kernel, unlike VMs which require hypervisors and guest OS overhead.',
    idealAnswer: 'Containers are isolated Linux processes, not virtual machines. They rely on: 1) Linux Namespaces for workspace isolation (PID for process IDs, NET for network interfaces, MNT for file mounts), 2) Control Groups (Cgroups) for resource constraint metering (limiting CPU, RAM, and disk I/O), and 3) OverlayFS / UnionFS for layered, read-only base image sharing with a thin writable Copy-on-Write (CoW) container layer on top.'
  },
  {
    id: 'topic-devops-3',
    subject: 'devops',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'DevOps - Kubernetes Architecture',
    question: 'Explain the Kubernetes Control Plane (API Server, etcd, Scheduler, Controller Manager) and Worker Node components (Kubelet, Kube-proxy).',
    keyConcepts: ['Kube-apiserver (central REST gateway)', 'etcd (distributed consistent key-value store for state)', 'Kube-scheduler (node placement)', 'Kubelet (pod lifecycle agent on node)', 'Kube-proxy (iptables/IPVS networking)'],
    tips: 'Mention that etcd uses the Raft consensus algorithm and only the API server talks directly to etcd.',
    idealAnswer: 'Control Plane: Kube-apiserver acts as the central REST API gateway; etcd stores all cluster state and configuration in a consistent key-value store; Kube-scheduler assigns pods to worker nodes based on resources and constraints; Controller Manager ensures actual cluster state matches desired state. Worker Nodes: Kubelet communicates with the API server to run pods via the Container Runtime (containerd); Kube-proxy manages network packet filtering and service IP load-balancing via iptables/IPVS.'
  },
  {
    id: 'topic-devops-4',
    subject: 'devops',
    ...
    question: 'How does Terraform manage cloud infrastructure? Explain Terraform State, State Locking, and Declarative vs Imperative IaC.',
    keyConcepts: ['Declarative HCL syntax', 'terraform.tfstate file (mapping real-world resources)', 'Remote backend with S3 & DynamoDB state locking', 'terraform plan vs apply', 'Drift detection'],
    tips: 'Contrast Terraform (Declarative - specify desired end state) with Ansible/scripts (Imperative - specify step-by-step commands).',
    idealAnswer: 'Terraform is a declarative IaC tool where you define desired target infrastructure in HCL code. The `terraform.tfstate` file maps code configurations to real-world cloud resources and tracks metadata. Remote backends (like S3 + DynamoDB) store state centrally and provide mutex locks to prevent race conditions during concurrent runs. `terraform plan` computes the diff (create, update, destroy), and `terraform apply` executes the API changes.'
  },
  {
    id: 'topic-devops-5',
    subject: 'devops',
    ...
    question: 'Compare Blue-Green, Canary, and Rolling Deployment strategies. When should you choose each?',
    keyConcepts: ['Rolling update (gradual pod replacement with zero downtime)', 'Blue-Green (identical parallel environments with instantaneous router switch)', 'Canary (routing small percentage 5% of live traffic to new version)', 'Instant rollback mechanisms'],
    tips: 'Explain that Blue-Green requires 2x infrastructure cost during rollout, while Canary minimizes user blast radius for critical regressions.',
    idealAnswer: 'Rolling Updates gradually replace old pods with new ones; it is resource-efficient but briefly runs mixed versions. Blue-Green maintains two identical environments (Blue=Live, Green=New); after testing Green, traffic is switched instantly via router/DNS (allows instant zero-downtime rollback but requires 2x infrastructure). Canary deploys the new version to a small subset (e.g. 5% traffic); if error rates stay healthy, it rolls out to 100%, isolating regression blast radius.'
  },
  {
    id: 'topic-devops-6',
    subject: 'devops',
    ...
    question: 'Explain the 3 Pillars of Observability: Metrics, Logs, and Distributed Traces. How do Prometheus, Grafana, and OpenTelemetry work together?',
    keyConcepts: ['Metrics (numeric time-series aggregated data)', 'Logs (discrete timestamped event records)', 'Distributed Tracing (spans tracking request path across microservices via trace_id)', 'OpenTelemetry standard collector'],
    tips: 'Clarify that while metrics tell you *that* a system is failing, distributed traces pinpoint *where* in a chain of 20 microservices latency occurred.',
    idealAnswer: '1) Metrics: Numeric time-series values aggregated over intervals (CPU, latency, error rate) scraped by Prometheus and visualized on Grafana. 2) Logs: Detailed structured event outputs (ELK / Loki) for forensic debugging. 3) Traces: Request propagation paths across microservices using a unique Trace ID and Span IDs via OpenTelemetry/Jaeger, pinpointing which downstream database or RPC caused latency spikes.'
  },
  {
    id: 'topic-devops-7',
    subject: 'devops',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'DevOps - Docker Optimization',
    question: 'How do you build secure, lightweight, and fast-building production Docker images using Multi-Stage builds and layer caching?',
    keyConcepts: ['Multi-stage Dockerfiles (build stage vs production runner stage)', 'Minimizing image size (Alpine / Distroless)', 'Layer ordering (copy package.json before source code)', 'Non-root user execution (`USER node`)'],
    tips: 'Explain that multi-stage builds leave heavy compilers, SDKs, and devDependencies out of the final runtime image.',
    idealAnswer: 'Best practices: 1) Multi-Stage Builds: Use a full SDK image (e.g., node:20-alpine) to build binaries and prune node_modules, then copy only the compiled output into a minimal runtime base (e.g., distroless or alpine). 2) Layer Caching: Place infrequently changing instructions (COPY package*.json, RUN npm install) before frequently changing source code. 3) Security: Run as non-root user and scan images with Trivy.'
  },
  {
    id: 'topic-devops-8',
    subject: 'devops',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'DevOps - SRE & Reliability',
    question: 'Define SLA, SLO, SLI, and Error Budgets in Site Reliability Engineering (SRE). How do they govern release velocity?',
    keyConcepts: ['SLI (Service Level Indicator: metric e.g. successful request %)', 'SLO (Service Level Objective: internal target e.g. 99.9% uptime)', 'SLA (Service Level Agreement: legal contract with penalties)', 'Error Budget (100% - SLO: fuel for rapid deployments)'],
    tips: 'Explain what happens when an Error Budget is exhausted: feature releases halt and engineering switches exclusively to stability/reliability bugs.',
    idealAnswer: 'SLI (Indicator) is a quantifiable metric measuring performance (e.g., latency < 200ms for 99% of requests). SLO (Objective) is the internal target threshold agreed upon with product teams (e.g., 99.9% availability). SLA (Agreement) is the external business commitment with financial penalties. Error Budget is (1 - SLO), representing allowable downtime; when the error budget is healthy, teams ship fast; when exhausted, feature deployments pause to focus on reliability.'
  },
  {
    id: 'topic-devops-9',
    subject: 'devops',
    ...
    question: 'How do you securely manage API keys, database credentials, and certificates across CI/CD and Kubernetes environments?',
    keyConcepts: ['Anti-pattern: Hardcoding or committing secrets in Git', 'HashiCorp Vault / AWS Secrets Manager', 'Kubernetes Secrets & External Secrets Operator', 'Automated rotation & Least Privilege injection'],
    tips: 'Explain that Kubernetes native base64 secrets are NOT encrypted; use Sealed Secrets, KMS envelope encryption, or External Secrets Operator.',
    idealAnswer: 'Never commit secrets to Git. Store secrets in centralized vaults (HashiCorp Vault, AWS Secrets Manager). In Kubernetes, use External Secrets Operator (ESO) or Sealed Secrets to sync vault secrets securely into runtime pods as environment variables or memory-mounted volumes. Enable automated secret rotation, dynamic credentials with short TTLs, and scan repositories with git-secrets/Gitleaks in CI.'
  },
  {
    id: 'topic-devops-10',
    subject: 'devops',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'DevOps - GitOps & ArgoCD',
    question: 'What is GitOps? How does ArgoCD use declarative Git repositories as the Single Source of Truth for Kubernetes clusters?',
    keyConcepts: ['Git as Single Source of Truth', 'Pull-based reconciliation vs Push-based CI scripts', 'Automatic drift correction (Self-healing)', 'Auditability & git commit rollback'],
    tips: 'Contrast traditional push pipelines (CI runner needs cluster admin kubeconfig) with pull GitOps (agent inside cluster syncs state).',
    idealAnswer: 'GitOps is an operational model where the entire desired system state is version-controlled in a Git repository. ArgoCD runs as a Kubernetes controller inside the cluster, continuously comparing the live cluster state against the Git manifest. If drift occurs (or a developer commits a change to Git), ArgoCD automatically reconciles and syncs the cluster. This improves security because CI pipelines do not need direct cluster administrative access.'
  },

  // ==========================================
  // 13. CYBERSECURITY (10 Questions)
  // ==========================================
  {
    id: 'topic-cybersecurity-1',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'easy',
    category: 'Cybersecurity - OWASP Top 10',
    question: 'What is SQL Injection (SQLi)? How does it occur, and why are Parameterized Queries (Prepared Statements) the definitive defense?',
    keyConcepts: ['SQL syntax string concatenation vulnerability', '\' OR 1=1 -- exploit payload', 'Prepared Statements & parameterized separation of code vs data', 'ORM protections and limitations (raw queries)'],
    tips: 'Explain that parameterized queries compile the SQL query template first, treating user input strictly as literal string values rather than executable code.',
    idealAnswer: 'SQL Injection occurs when untrusted user input is concatenated directly into SQL queries, altering the command structure (e.g., `admin\' OR \'1\'=\'1`). Prepared Statements (Parameterized Queries) compile the database query structure beforehand and send parameters over a separate protocol channel, ensuring the database treats user input strictly as literal data rather than executable SQL commands.'
  },
  {
    id: 'topic-cybersecurity-2',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Web Vulnerabilities',
    question: 'Compare Stored XSS, Reflected XSS, and DOM-based XSS. What defenses (CSP, Escaping, HttpOnly cookies) prevent them?',
    keyConcepts: ['Stored XSS (malicious script saved to DB)', 'Reflected XSS (payload in URL query parameters)', 'DOM XSS (client-side JS `innerHTML` manipulation)', 'Content Security Policy (CSP)', 'HttpOnly & Secure cookie flags'],
    tips: 'Highlight that `HttpOnly` prevents JavaScript from accessing session tokens via `document.cookie`.',
    idealAnswer: 'Stored XSS permanently stores malicious JavaScript in a database and executes when other users view it. Reflected XSS reflects script payloads via query parameters or form submissions. DOM XSS executes entirely client-side when vulnerable JS injects unsanitized user input into dangerous sinks like `innerHTML`. Defenses: Context-aware HTML escaping, Content Security Policy (CSP) headers restricting script sources, and `HttpOnly` cookies to protect tokens from theft.'
  },
  {
    id: 'topic-cybersecurity-3',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Auth & Session Security',
    question: 'How do Cross-Site Request Forgery (CSRF) attacks operate, and how do SameSite Cookie flags and Anti-CSRF tokens stop them?',
    keyConcepts: ['Browser automatic cookie submission on cross-origin requests', 'SameSite Cookie attribute (Strict, Lax, None)', 'Anti-CSRF synchronizer token pattern', 'Custom request headers (`X-Requested-With`)'],
    tips: 'Emphasize that SameSite=Lax (default in modern browsers) stops most top-level POST CSRF attacks, but state-changing actions should still require Anti-CSRF tokens or custom headers.',
    idealAnswer: 'CSRF exploits the browser\'s behavior of automatically attaching stored authentication cookies to cross-origin requests made to target domains. Mitigation: 1) SameSite cookies (`SameSite=Strict` or `Lax`) prevent cookies from being sent in cross-site requests, 2) Anti-CSRF Synchronizer Tokens: A cryptographically unique token generated per session is validated on state-changing requests (POST/PUT/DELETE), which cross-origin attackers cannot read or forge.'
  },
  {
    id: 'topic-cybersecurity-4',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Cryptography',
    question: 'Explain the difference between Symmetric Encryption, Asymmetric Encryption, and Cryptographic Hashing (with Salt).',
    keyConcepts: ['Symmetric (AES - single shared secret key for bulk data)', 'Asymmetric (RSA/ECC - public/private key pair for key exchange & signing)', 'Cryptographic Hash (SHA-256 vs Argon2/Bcrypt for passwords)', 'Salt & Pepper to thwart Rainbow Tables'],
    tips: 'Emphasize: NEVER use fast general-purpose hashes like SHA-256 or MD5 for password storage; always use slow, memory-hard algorithms like Bcrypt or Argon2id.',
    idealAnswer: 'Symmetric encryption (AES-256) uses the same key for encryption and decryption (fast, used for data-at-rest and bulk payloads). Asymmetric encryption (RSA, ECC) uses a Public Key for encryption/verification and Private Key for decryption/signing (used for TLS handshakes and digital signatures). Cryptographic Hashing is a one-way mathematical function; passwords must be hashed using adaptive, salted algorithms (Argon2id, Bcrypt) to prevent brute-force and rainbow table attacks.'
  },
  {
    id: 'topic-cybersecurity-5',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Modern Authentication',
    question: 'How does OAuth 2.0 Authorization Code Flow with PKCE work, and what is the difference between OAuth 2.0 and OpenID Connect (OIDC)?',
    keyConcepts: ['OAuth 2.0 (Delegated Authorization framework)', 'OpenID Connect (Authentication layer on top of OAuth providing ID Token JWT)', 'PKCE (Proof Key for Code Exchange)', 'Code verifier and code challenge preventing interception'],
    tips: 'Remember: OAuth 2.0 issues Access Tokens (what can I access?), while OIDC issues ID Tokens (who are you?).',
    idealAnswer: 'OAuth 2.0 is an authorization framework allowing third parties to access user resources without passwords. OIDC is an identity layer built on top of OAuth 2.0 that introduces an ID Token (JWT) for authentication. The Authorization Code Flow with PKCE (Proof Key for Code Exchange) protects SPAs/mobile apps: the client generates a random `code_verifier` and hashed `code_challenge`, preventing malicious apps from intercepting authorization codes.'
  },
  {
    id: 'topic-cybersecurity-6',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'hard',
    category: 'Cybersecurity - JWT Security',
    question: 'What are the architectural vulnerabilities and security trade-offs of using JSON Web Tokens (JWT) for session management?',
    keyConcepts: ['JWT structure (Header.Payload.Signature)', 'Algorithm confusion attack (`alg: "none"` or HMAC/RSA switch)', 'Token revocation & stateless invalidation challenges', 'XSS token theft vs CSRF cookie storage'],
    tips: 'Discuss how to handle logout/revocation with stateless JWTs: short expiration TTL (5-15 min) + sliding refresh tokens stored in DB/Redis blacklist.',
    idealAnswer: 'JWTs are stateless signed tokens containing claims. Vulnerabilities include: 1) Algorithm confusion (forcing `none` algorithm or verifying RS256 with HMAC secret), 2) Sensitive data leakage (payload is Base64 encoded, not encrypted), 3) Inability to instantly revoke tokens on logout/compromise. Solution: Keep access token TTL very short (5-15 min), rotate refresh tokens stored in `HttpOnly` Secure cookies, and maintain a Redis blacklist for revoked tokens.'
  },
  {
    id: 'topic-cybersecurity-7',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'hard',
    category: 'Cybersecurity - Zero Trust Architecture',
    question: 'What is Zero Trust Architecture? Explain the core principles: "Never Trust, Always Verify" and Micro-segmentation.',
    keyConcepts: ['Perimeter security vs Zero Trust model', 'Continuous identity & device posture authentication', 'Principle of Least Privilege & RBAC/ABAC', 'Micro-segmentation and mTLS between microservices'],
    tips: 'Contrast the legacy "castle and moat" network perimeter with Zero Trust where inside the network is assumed compromised.',
    idealAnswer: 'Zero Trust assumes the internal network is already breached. Principles: 1) Never Trust, Always Verify: Every user, service, and API request is authenticated and authorized dynamically. 2) Least Privilege: Access is granted just-in-time with minimal scope using RBAC/ABAC. 3) Micro-segmentation: Isolating services in software-defined network perimeters with Mutual TLS (mTLS) to prevent lateral movement by attackers.'
  },
  {
    id: 'topic-cybersecurity-8',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'hard',
    category: 'Cybersecurity - Network & API Security',
    question: 'How do you defend high-throughput APIs against Distributed Denial of Service (DDoS) and API Abuse?',
    keyConcepts: ['Layer 3/4 SYN floods vs Layer 7 HTTP application floods', 'Rate limiting algorithms (Token Bucket, Leaky Bucket)', 'Web Application Firewall (WAF) rule sets', 'Anycast network scrubbing (Cloudflare / AWS Shield)'],
    tips: 'Differentiate L3/L4 volumetric attacks (mitigated by Anycast edge routing) from L7 application attacks (mitigated by WAFs, CAPTCHAs, and IP/Token rate limiting).',
    idealAnswer: 'Defenses: 1) Edge Network Scrubbing: Anycast DNS and CDN networks (AWS Shield, Cloudflare) absorb L3/L4 volumetric traffic. 2) WAF (Web Application Firewall): Blocks malicious L7 bot signatures and anomalous HTTP request rates. 3) Rate Limiting & Throttling: Implement Token Bucket algorithms per API key/IP in Redis. 4) Schema validation, query depth limiting in GraphQL, and aggressive edge caching.'
  },
  {
    id: 'topic-cybersecurity-9',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Server-Side Request Forgery',
    question: 'What is Server-Side Request Forgery (SSRF)? How did SSRF lead to major cloud breaches (like Capital One), and how is it mitigated?',
    keyConcepts: ['Server fetching attacker-supplied URL', 'Accessing cloud metadata service (169.254.169.254)', 'IMDSv2 session token mitigation', 'Strict URL allowlisting & internal IP blacklisting'],
    tips: 'Mention AWS IMDSv2 which requires a `PUT` request with a token header, preventing simple SSRF via HTTP GET requests.',
    idealAnswer: 'SSRF occurs when a web server accepts a user-provided URL and fetches it without validation, allowing attackers to access internal intranet services or the Cloud Instance Metadata Service (169.254.169.254) to steal IAM credentials. Mitigations: 1) Require AWS IMDSv2 (enforcing session tokens), 2) Enforce strict domain allowlists, 3) Block private/loopback IP ranges (RFC 1918) before making outbound HTTP requests.'
  },
  {
    id: 'topic-cybersecurity-10',
    subject: 'cybersecurity',
    role: 'software-developer',
    type: 'technical',
    difficulty: 'medium',
    category: 'Cybersecurity - Threat Modeling',
    question: 'Explain the STRIDE Threat Modeling framework with an example for each category in a web application.',
    keyConcepts: ['Spoofing (identity falsification)', 'Tampering (data modification)', 'Repudiation (denying actions - lack of audit logs)', 'Information Disclosure (data leakage)', 'Denial of Service (availability crash)', 'Elevation of Privilege (unauthorized admin access)'],
    tips: 'Walk through an e-commerce checkout flow applying each letter of STRIDE systematically.',
    idealAnswer: 'STRIDE categories: 1) Spoofing: Impersonating a user (fixed with MFA/OAuth). 2) Tampering: Modifying transaction amounts in client requests (fixed with cryptographic signatures). 3) Repudiation: Denying performing a transfer (fixed with immutable audit logs). 4) Information Disclosure: Leaking PII/credit cards (fixed with TLS/KMS encryption). 5) Denial of Service: Crashing payment APIs (fixed with rate limiters). 6) Elevation of Privilege: User elevating to admin (fixed with strict RBAC authorization).'
  },

  // ==========================================
  // 14. GIT & VERSION CONTROL (10 Questions)
  // ==========================================
  {
    id: 'topic-git-1',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'Git - Core Internals',
    question: 'How does Git store data under the hood? Explain Blobs, Trees, Commits, and Annotated Tags in the `.git/objects` directory.',
    keyConcepts: ['Directed Acyclic Graph (DAG)', 'Content-addressable storage (SHA-1 / SHA-256 hash)', 'Blob (file content)', 'Tree (directory structure & filenames)', 'Commit (tree pointer + author + parent commit)'],
    tips: 'Clarify that Git does NOT store file diffs/deltas between versions; it stores complete snapshots (with deduplicated zlib-compressed objects).',
    idealAnswer: 'Git is a content-addressable key-value store structured as a Directed Acyclic Graph (DAG). 1) Blobs store raw file content (identified by SHA hash of contents). 2) Trees represent directories, referencing blob hashes with filenames and permissions. 3) Commits point to a root Tree object, author/committer metadata, timestamp, and parent commit hashes. 4) Annotated Tags point to specific commit hashes with a message and GPG signature.'
  },
  {
    id: 'topic-git-2',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Git - Merge vs Rebase',
    question: 'Compare `git merge` vs `git rebase`. What is the Golden Rule of Git Rebase, and how does Interactive Rebase (`git rebase -i`) clean up commit history?',
    keyConcepts: ['Merge: Creates 3-way merge commit preserving exact history', 'Rebase: Rewrites commit history on top of base branch (linear history)', 'Golden Rule: Never rebase shared/public branches', 'Interactive Rebase: Squash, fixup, reword, drop'],
    tips: 'Explain that rebasing rewrites commit SHA hashes; rebasing a shared master branch will break history for all collaborators.',
    idealAnswer: '`git merge` combines branches by creating a new merge commit, preserving exact chronological branching history. `git rebase` rewrites history by moving feature branch commits on top of the target branch, producing a clean, linear commit history without merge bubbles. The Golden Rule of Rebase: Never rebase commits that have been pushed to a public/shared branch. `git rebase -i` allows developers to squash dirty WIP commits, reword messages, and drop unwanted changes before merging.'
  },
  {
    id: 'topic-git-3',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'Git - Undoing Changes',
    question: 'Explain the differences between `git reset --soft`, `git reset --mixed`, and `git reset --hard`. When should you use `git revert` instead?',
    keyConcepts: ['`git reset --soft` (moves HEAD, leaves changes staged in index)', '`git reset --mixed` (default: un-stages changes to working directory)', '`git reset --hard` (destroys changes in working tree and index)', '`git revert` (creates a safe inverse commit for public branches)'],
    tips: 'Emphasize that `git reset` modifies commit history locally, whereas `git revert` is safe for public shared branches because it adds a new undo commit.',
    idealAnswer: '`git reset` moves the HEAD pointer backward: `--soft` moves HEAD but keeps changes staged in the index; `--mixed` (default) moves HEAD and unstages changes to the working directory; `--hard` completely discards changes in both index and working tree. For commits already pushed to public remotes, `git revert <hash>` must be used instead because it safely creates a new forward commit that inverts the changes without rewriting history.'
  },
  {
    id: 'topic-git-4',
    subject: 'git',
    ...
    question: 'How do Merge Conflicts happen in Git, and what steps do you take to resolve complex 3-Way Merge conflicts effectively?',
    keyConcepts: ['Common ancestor base', 'Conflicting modifications on identical lines across branches', 'Merge conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`)', 'Diff3 view and verification via tests'],
    tips: 'Explain that Git uses a 3-way merge algorithm comparing Branch A, Branch B, and their common merge base ancestor.',
    idealAnswer: 'Merge conflicts occur when two branches modify the same line of a file differently since their common merge base ancestor, preventing Git from resolving changes automatically. Resolution steps: 1) Open conflicting files identified by `git status`, 2) Analyze conflict markers (`<<<<<<< HEAD` vs `>>>>>>> incoming`), 3) Consult team members if intent is ambiguous, 4) Edit the file to desired final code, 5) Run test suites to verify correctness, 6) Stage with `git add` and commit with `git commit`.'
  },
  {
    id: 'topic-git-5',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Git - Disaster Recovery & Reflog',
    question: 'How does `git reflog` save developers from accidental data loss (like an unintentional `git reset --hard` or deleted branch)?',
    keyConcepts: ['Reference log (reflog: local chronological log of HEAD updates)', 'Dangling / orphan commits', 'Recovering lost state (`git reset --hard HEAD@{n}` or `git checkout -b rescue <hash>`)', 'Git garbage collection (`git gc`) TTL'],
    tips: 'Reflog records all HEAD position changes (checkout, commit, rebase, reset); objects remain in Git storage until garbage collected (typically 30-90 days).',
    idealAnswer: '`git reflog` records every change made to the local `HEAD` reference (commits, checkouts, resets, rebases), even if commits are no longer part of any named branch. If a developer accidentally runs `git reset --hard` or deletes a branch, they can run `git reflog` to find the lost commit hash and restore it immediately using `git branch recovery-branch <commit-hash>` or `git reset --hard HEAD@{n}`.'
  },
  {
    id: 'topic-git-6',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Git - Branching Strategies',
    question: 'Compare GitFlow, GitHub Flow, and Trunk-Based Development. Which is best suited for high-velocity CI/CD environments?',
    keyConcepts: ['GitFlow (feature, develop, release, hotfix, main branches)', 'GitHub Flow (feature branches merged directly to main via PR)', 'Trunk-Based Development (short-lived branches < 1 day, feature flags)', 'CI/CD alignment'],
    tips: 'Explain that Trunk-Based Development combined with feature flags minimizes merge debt and is standard across high-performing engineering teams.',
    idealAnswer: 'GitFlow uses complex long-lived branches (develop, release, hotfix, master), which creates merge friction and slows deployments. GitHub Flow uses lightweight feature branches deployed to main after PR reviews. Trunk-Based Development has developers merge short-lived branches (under 24h) directly into trunk (main) multiple times daily, gating unfinished features with Feature Flags to maximize CI/CD velocity and eliminate merge hell.'
  },
  {
    id: 'topic-git-7',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'easy',
    category: 'Git - Stashing & Cherry-Picking',
    question: 'Explain `git stash` and `git cherry-pick`. What are their real-world engineering use cases?',
    keyConcepts: ['`git stash push -u` & `git stash pop`', 'Stash stack management (`git stash list`, `git stash apply`)', '`git cherry-pick <commit-hash>`', 'Hotfixing specific commits across release branches'],
    tips: 'Mention `-u` (include untracked files) when stashing so newly created files are not left behind in the workspace.',
    idealAnswer: '`git stash` temporarily saves uncommitted working directory and staged modifications to an internal stack, reverting the working tree to a clean HEAD state so you can pull changes or switch branches (restored with `git stash pop`). `git cherry-pick <commit-hash>` applies the exact changes from a specific commit on one branch onto your current branch without merging the entire branch, ideal for applying critical hotfixes across release branches.'
  },
  {
    id: 'topic-git-8',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Git - Automation & Hooks',
    question: 'How do Git Hooks and tools like Husky / lint-staged enforce code quality and prevent committing secrets before pushing?',
    keyConcepts: ['Client-side hooks (`pre-commit`, `pre-push`, `commit-msg`)', 'Server-side hooks (`pre-receive`, `update`)', 'Husky + lint-staged automation', 'Gitleaks / Secret scanning prevention'],
    tips: 'Note: Client-side hooks can be bypassed with `--no-verify`, so server-side CI/CD checks remain mandatory as final defense.',
    idealAnswer: 'Git hooks are executable scripts in `.git/hooks` triggered by events like commits and pushes. Tools like Husky manage hooks via version control; `lint-staged` runs linters (ESLint, Prettier) and unit tests exclusively on staged files during `pre-commit`. Secret scanners (Gitleaks, TruffleHog) run in hooks to block commits containing private keys or credentials. Server-side CI checks enforce these rules definitively against `--no-verify` bypasses.'
  },
  {
    id: 'topic-git-9',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'hard',
    category: 'Git - Debugging & Bisect',
    question: 'How does `git bisect` use binary search to locate the exact commit that introduced a production regression bug?',
    keyConcepts: ['Binary search over commit history (O(log N))', '`git bisect start`, `git bisect bad`, `git bisect good`', 'Automated bisect with scripts (`git bisect run ./test.sh`)', '`git bisect reset`'],
    tips: 'Highlight that `git bisect run <script>` can test hundreds of commits completely automatically in seconds.',
    idealAnswer: '`git bisect` performs a binary search through commit history: 1) Run `git bisect start`, 2) Mark current broken commit with `git bisect bad`, 3) Mark a known working historical commit with `git bisect good <hash>`, 4) Git automatically checks out the midpoint commit for testing, 5) You mark it as good/bad until Git identifies the exact faulty commit. This can be fully automated using `git bisect run npm test` in O(log N) iterations.'
  },
  {
    id: 'topic-git-10',
    subject: 'git',
    ...
    type: 'technical',
    difficulty: 'medium',
    category: 'Git - Monorepo & Submodules',
    question: 'Compare Git Submodules, Git Subtrees, and Monorepo tooling (Turborepo / Nx) for managing shared multi-project codebases.',
    keyConcepts: ['Git Submodules (pinned commit pointer to external repo)', 'Git Subtrees (nested repository history merged directly)', 'Monorepo architecture with package managers (pnpm / Turborepo workspaces)', 'Dependency caching and unified versioning'],
    tips: 'Explain that Submodules frequently confuse developers due to detached HEADs, whereas modern Monorepo workspaces offer smoother shared package development.',
    idealAnswer: 'Git Submodules embed an external Git repository at a specific commit hash, requiring separate clone/pull flags (`--recurse-submodules`). Git Subtrees merge external repo history directly into a subdirectory. For large multi-service systems, Monorepos managed with tools like Turborepo, Nx, or pnpm workspaces store all apps/packages in one repo with centralized tooling, shared dependencies, atomic cross-package commits, and intelligent task build caching.'
  }
];
