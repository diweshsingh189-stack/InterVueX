...
import { COMPANY_QUESTIONS } from './companyQuestionsData.js';
import { TOPIC_QUESTIONS } from './topicQuestionsData.js';

export { TOPIC_QUESTIONS };
...
export function getQuestionsForInterview(roleId, typeId, difficultyId, count = 5, companyId = 'all') {
  ...
  if (companyId && companyId !== 'all') {
    const norm = String(companyId).toLowerCase().trim().replace(/[\s-_]+/g, '');
    companySpecific = QUESTIONS_DATABASE.filter(q => q.company && String(q.company).toLowerCase().trim().replace(/[\s-_]+/g, '') === norm);
  }

  let filtered = [];
  if (companySpecific.length > 0) {
    filtered = companySpecific.filter(q => {
      ...
      const roleMatch = (typeId === 'coding' || typeId === 'hr' || typeId === 'behavioral') 
        ? true 
        : (q.role === roleId || !q.role);

      const diffMatch = difficultyId 
        ? (q.difficulty === difficultyId || q.difficulty === 'medium') 
        : true;

      return typeMatch && roleMatch && diffMatch;
    });

    // If filtered is less than count, supplement with remaining questions from that company
    if (filtered.length < count) {
      const remainingCompany = companySpecific.filter(q => !filtered.some(f => f.id === q.id));
      filtered = [...filtered, ...remainingCompany];
    }
  } else {
    filtered = QUESTIONS_DATABASE.filter(q => {
      const typeMatch = (typeId === 'mixed') 
        ? true 
        : (typeId === 'coding' ? q.type === 'coding' : q.type === typeId);
  ...
  // If filtered pool is smaller than desired count, supplement from universal database
  if (filtered.length < count) {
